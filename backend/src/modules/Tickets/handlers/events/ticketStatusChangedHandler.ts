...
import { TicketStatusChangedEvent } from '../../events/TicketStatusChangedEvent';
...
export const ticketStatusChangedHandler = async (
  event: EventBridgeEvent<'ticket.status_changed', TicketStatusChangedEvent>
): Promise<void> => {
  try {
    ...
    console.log(`📡 Cambio de estado de ticket enviado vía WebSocket a ${sentCount} conexión(es)`);
  } catch (error) {
    console.error('❌ Error procesando evento ticket.status_changed:', error);
    throw error;
  }
};
