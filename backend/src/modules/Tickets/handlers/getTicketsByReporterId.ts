...
/**
 * Handler para obtener todos los tickets creados por un usuario
 * @param event - Evento de API Gateway
 * @returns Respuesta con lista de tickets paginada
 */
const getTicketsByReporterIdHandler = async (event: APIGatewayProxyEvent) => {
  ...
  const result = await ticketsService.getTicketsByReporterId(user.id, page, limit);
...
export const handler = handlerWrapper(getTicketsByReporterIdHandler);
