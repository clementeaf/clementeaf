...
import { ClientUpdatedEvent } from '../../events/ClientUpdatedEvent';
...
export const clientUpdatedHandler = async (
  event: EventBridgeEvent<'client.updated', ClientUpdatedEvent>
): Promise<void> => {
  try {
    ...
    console.log(`📡 Cliente actualizado enviado vía WebSocket a ${sentCount} conexión(es)`);
  } catch (error) {
    console.error('❌ Error procesando evento client.updated:', error);
    throw error;
  }
};
