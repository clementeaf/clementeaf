...
import { BranchUpdatedEvent } from '../../events/BranchUpdatedEvent';
...
export const branchUpdatedHandler = async (
  event: EventBridgeEvent<'branch.updated', BranchUpdatedEvent>
): Promise<void> => {
  try {
    ...
    console.log(`📡 Sucursal actualizada enviada vía WebSocket a ${sentCount} conexión(es)`);
  } catch (error) {
    console.error('❌ Error procesando evento branch.updated:', error);
    throw error;
  }
};
