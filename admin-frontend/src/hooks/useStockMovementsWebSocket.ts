...
export const useStockMovementsWebSocket = (options: UseStockMovementsWebSocketOptions) => {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          const data: StockMovementsWebSocketMessage = JSON.parse(event.data);
          
          if (data.action === 'stock_movement_created' && data.productId) {
            // Invalidar queries relacionadas con el producto
            queryClient.invalidateQueries({ queryKey: ['productHistory', data.productId] });
            queryClient.invalidateQueries({ queryKey: ['products'] });
            onMovementCreated?.(data);
          }
        } catch (error) {
          console.error('❌ [STOCK MOVEMENTS WS] Error parseando mensaje:', error);
        }
      };

      ws.onerror = (error) => {
        console.error('❌ [STOCK MOVEMENTS WS] Error en WebSocket:', error);
      ...
      console.error('❌ [STOCK MOVEMENTS WS] Error creando WebSocket:', error);
      onError?.(error instanceof Error ? error : new Error('Failed to create WebSocket'));
    }
  }, [currentUser?.id, queryClient, onMovementCreated, onError]);
